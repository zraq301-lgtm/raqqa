...
// استيراد الأقسام من المسار: src/pages/SwingPage
// ملاحظة: إذا كان هذا الملف (Swing.jsx) موجوداً في src/pages/
// فإن الاستدعاء يكون من './SwingPage/...'
  ...
  return (
    <div style={{ direction: 'rtl', backgroundColor: '#fff9fb', minHeight: '100vh', fontFamily: 'system-ui' }}>
      ...
      <header style={{
        ...
        <button onClick={() => setIsChatOpen(true)} style={{ background: '#D81B60', border: 'none', color: '#fff', width: '40px', height: '40px', borderRadius: '50%', cursor: 'pointer' }}>
          <Sparkles size={20} />
        </button>
      </header>
      ...
      <nav style={{
        ...
        backgroundColor: '#fff', borderBottom: '1px solid #eee', padding: '10px 0', 
        overflowX: 'auto', display: 'flex', zIndex: 1050, whiteSpace: 'nowrap'
      }}>
        {CATEGORIES.map((cat, index) => (
          <Link key={index} to={cat.path} style={{
            textDecoration: 'none', padding: '8px 15px', margin: '0 5px',
            borderRadius: '20px', fontSize: '13px', fontWeight: 'bold',
            color: location.pathname === cat.path ? '#fff' : '#D81B60',
            backgroundColor: location.pathname === cat.path ? '#D81B60' : '#fce4ec',
          }}>
            {cat.ar}
          </Link>
        ))}
      ...
      <main style={{ paddingTop: '140px', paddingBottom: '40px', maxWidth: '500px', margin: '0 auto', paddingLeft: '15px', paddingRight: '15px' }}>
        <Routes>
          <Route path="/" element={
            <>
              {/* صندوق النشر */}
              <div style={{ backgroundColor: '#fff', borderRadius: '15px', padding: '15px', marginBottom: '20px', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' }}>
                <textarea 
                  value={newPostContent}
                  onChange={(e) => setNewPostContent(e.target.value)}
                  placeholder="ماذا يدور في ذهنكِ؟"
                  style={{ width: '100%', border: 'none', outline: 'none', resize: 'none', minHeight: '60px' }}
                />
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px' }}>
                  <input type="file" id="filePost" hidden onChange={(e) => setSelectedImage(e.target.files[0])} />
                  <label htmlFor="filePost" style={{ cursor: 'pointer' }}><ImageIcon size={20} color="#D81B60" /></label>
                  <button onClick={handlePublish} disabled={isPublishing} style={{ backgroundColor: '#D81B60', color: '#fff', border: 'none', padding: '6px 20px', borderRadius: '20px', cursor: 'pointer' }}>
...
const PostCard = ({ post }) => {
  ...
  return (
    <div style={{ backgroundColor: '#fff', borderRadius: '15px', padding: '15px', marginBottom: '15px', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' }}>
      <p style={{ fontSize: '15px', color: '#333' }}>{post.content}</p>
      {post.file && <img src={post.file} style={{ width: '100%', borderRadius: '10px', marginTop: '10px' }} alt="post" />}
      
      <div style={{ display: 'flex', gap: '20px', marginTop: '15px', borderTop: '1px solid #f5f5f5', paddingTop: '10px' }}>
        <button onClick={() => setShowInput(!showInput)} style={{ background: 'none', border: 'none', color: '#666', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}>
      ...
      {/* عرض التعليقات */}
      {comments.map((c, i) => (
        <div key={i} style={{ backgroundColor: '#f8f8f8', padding: '8px 12px', borderRadius: '10px', marginTop: '8px', fontSize: '13px' }}>
          {c}
        </div>
      ))}
      ...
      {showInput && (
        <div style={{ display: 'flex', gap: '5px', marginTop: '10px' }}>
          <input 
            ...
            style={{ flex: 1, backgroundColor: '#f0f2f5', border: 'none', padding: '8px 12px', borderRadius: '20px', fontSize: '13px' }}
          />
          <button onClick={handleSend} style={{ backgroundColor: '#D81B60', color: '#fff', border: 'none', borderRadius: '50%', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Send size={14} />
          </button>
        </div>
      )}
