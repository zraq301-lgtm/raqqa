...
const LittleOnesAcademy = () => {
  ...
  // 🎯 جلب البيانات بناءً على الاسم العربي الصحيح للقسم المتوافق مع السيرفر والملفات
  const PAGE_NAME = "أكاديمية الصغار"; 
  ...
  if (loading) return <div className="loading-screen">لحظات من أجل صغارنا... ✨</div>;

  return (
    <div className="main-wrapper" dir="rtl">
      
      {/* الترحيب الثابت المخصص لقسم أكاديمية الصغار */}
      <div className="fixed-welcome">
        ✨ عالم طفلكِ يبدأ من هنا.. خطوات مدروسة، وأفكار ملهمة لبناء جيل مبدع، واعي، ومستعد للمستقبل. ✨
      </div>
            ...
            return (
              <div key={post.id} className="article-container">
                <div className={`card ${post.isMediaOnly ? 'media-only-card' : ''}`}>
                  ...
                  {/* ترويسة تعرض رقم الـ ID الحقيقي المستلم من قاعدة البيانات لسهولة التحكم */}
                  ...
                  {!featuredVideo && featuredImage && (
                    <div className="main-featured-image">
                      <img src={featuredImage} alt="LittleOnesAcademy" />
                    </div>
                  )}
...
export default LittleOnesAcademy;
