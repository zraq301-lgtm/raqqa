...
const SoulsLounge = () => {
    ...
    try {
      ...
      if (result) {
        ...
        const formattedPosts = rawItems.map((item, index) => {
          ...
          // فحص حجم الكارت بشكل حيوي لتفادي المسافات البيضاء الفارغة
        ...
        // ترتيب المنشورات من الأحدث إلى الأقدم تصاعدياً
  ...
  const handleShare = async (title) => {
    ...
    try {
      await Share.share({
        title: cleanTitle,
        text: `شاركي هذا الموضوع من رواق الأرواح في تطبيق رقة: ${cleanTitle}`,
        url: APP_LINK,
  ...
  if (loading) return <div className="loading-screen">لحظات سكون مطمئنة... ✨</div>;

  return (
    <div className="main-wrapper" dir="rtl">
      
      {/* الترحيب الثابت والأنيق الحاضن للهوية البصرية للقسم */}
      <div className="fixed-welcome">
        <div className="welcome-border">
          ✨ مرحباً بكم في رواق الأرواح.. حيث للوفاء موطن ✨
        </div>
      </div>
      
      <div className="container">
        {articles.length === 0 ? (
          <div style={{ textAlign: 'center', color: '#8d6e63', marginTop: '50px', fontFamily: 'Tajawal' }}>لا توجد منشورات حالياً في هذا الرواق</div>
        ) : (
          articles.map((post) => {
            ...
            return (
              <div key={post.id} className="article-container">
                <div className={`card ${post.isMediaOnly ? 'media-only-card' : ''}`}>
                  ...
                  {/* شارة الـ ID الحقيقي لتتبع العناصر بيسر من الإدارة */}
                  ...
                  {!featuredVideo && featuredImage && (
                    <div className="main-featured-image">
                      <img src={featuredImage} alt="SoulsLounge" />
                    </div>
                  )}
      ...
      <style jsx global>{`
        ...
        /* ضبط الكروت التي تحتوي على وسائط فقط */
