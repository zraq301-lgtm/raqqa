...
const PassionsCrafts = () => {
  ...
  // 🎯 جلب البيانات بناءً على الاسم العربي المخصص لقسم "شغف وحرف" ليتطابق مع السيرفر
  const PAGE_NAME = "شغف وحرف"; 
    ...
    try {
      ...
      if (result) {
        ...
        const formattedPosts = rawItems.map((item, index) => {
          ...
          // 🎯 استخراج الـ ID الحقيقي المخزن بقاعدة البيانات لتأمين الحذف والتعديل
          const databaseId = item.id || item.lastUpdated || `post_${index}`;

          // قياس حجم الكارت ديناميكياً (منشور ميديا صرف أم ميديا ونص)
        ...
        // ترتيب المنشورات لعرض الأحدث في الأعلى دائماً
  ...
  const handleShare = async (title) => {
    ...
    try {
      await Share.share({
        title: cleanTitle,
        text: `شاركي هذه المقالة من تطبيق رقة: ${cleanTitle}`,
        url: APP_LINK,
  ...
  if (loading) return <div className="loading-screen">لحظات من الشغف والإبداع... ✨</div>;

  return (
    <div className="main-wrapper" dir="rtl">
      
      {/* الترحيب الثابت المخصص والمميز بإطاره الأبيض الشفاف */}
      <div className="fixed-welcome">
        <div className="welcome-border">
          ✨ مرحباً بكِ في عالم الحِرف.. حيث نُحوّل شغف يديكِ إلى بصمةِ إتقان لا تشبه أحداً. ✨
        </div>
      </div>
            ...
            return (
              <div key={post.id} className="article-container">
                <div className={`card ${post.isMediaOnly ? 'media-only-card' : ''}`}>
                  ...
                  {/* ترويسة تعرض المعرف الرقمي الأصلي للمنشور المستلم من السيرفر */}
                  ...
                  {!featuredVideo && featuredImage && (
                    <div className="main-featured-image">
                      <img src={featuredImage} alt="PassionsCrafts" />
                    </div>
                  )}
      ...
      <style jsx global>{`
        ...
        .welcome-border { border: 2px solid rgba(255, 255, 255, 0.6); border-radius: 12px; padding: 8px 15px; color: white; text-align: center; font-size: 0.95rem; font-weight: 700; letter-spacing: 0.5px; line-height: 1.4; }
        ...
        /* معالجة أحجام كروت الميديا الصرف تلقائياً */
        ...
        /* شارة الـ ID الأصلي بقاعدة البيانات */
