...
// استيراد خدمة الميديا من المسار المطلوب
...
const App = () => {
  ...
  const getAIAnalysis = async (customPrompt = null) => {
    ...
    // تخصص استشاري أطفال
    const systemExpertise = "بصفتك استشاري أطفال متخصص وخبير في التربية وصحة الطفل، قدم لي نصائح تربوية وطبية دقيقة بناءً على أحدث الأبحاث.";
    const promptMessage = customPrompt 
      ? `${systemExpertise} سؤالي هو: ${customPrompt}`
      : `${systemExpertise} لقد تابعت المهام التالية للأطفال: (${selectedOnes.join(" - ")}) في قسم ${currentList.title}. حلل الحالة وقدم نصائح احترافية.`;
  ...
  // تفعيل الكاميرا والمعرض باستخدام MediaService
  ...
  return (
    <div style={styles.container}>
      ...
      <header style={styles.header}>
        ...
        <p>دليل الأم الواعية للتربية والصحة</p>
      </header>
                  ...
                  <div style={styles.msgBubble}>
                    ...
                    <div style={styles.msgFooter}>
                      ...
                      <div style={{display: 'flex', gap: '5px'}}>
                        {msg.sender === 'ai' && (
                          <button onClick={() => saveReply(msg)} style={styles.saveBtn}><i className="fas fa-bookmark"></i></button>
                        )}
...
const styles = {
  ...
  specialistBtn: { padding: '12px 25px', borderRadius: '25px', border: 'none', background: '#2e8b57', color: 'white', fontWeight: 'bold', cursor: 'pointer' },
