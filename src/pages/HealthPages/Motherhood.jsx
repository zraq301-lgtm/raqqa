...
// استيراد الدوال من المسار المحدد 
...
const App = () => {
  ...
  const getAIAnalysis = async (customPrompt = null) => {
    ...
    // تحديث تخصص الذكاء الاصطناعي ليكون استشاري أطفال باستخدام مجموعات الأطباء والمراجع المتخصصة
    const systemExpertise = "بصفتك استشاري طب أطفال وخبير تربوي، معتمداً على توصيات الأكاديمية الأمريكية لطب الأطفال ومجموعات استشاريي الأطفال، قدم نصيحة طبية وسلوكية دقيقة."; [cite: 13, 15]

    const promptMessage = customPrompt 
      ? `${systemExpertise} سؤالي هو: ${customPrompt}` [cite: 15]
      : `${systemExpertise} لقد تابعت المهام التالية مع طفلي: (${selectedOnes.join(" - ")}) في قسم ${currentList.title}. حلل حالتي وقدم نصائح شاملة.`; [cite: 16]
   ...
   * تم تعديلها لتستخدم المسار والوظائف المطلوبة 
   */
  const handleMediaAction = async (type) => {
    try {
        ...
        const base64Data = type === 'camera' 
            ? await takePhoto() 
            : await fetchImage(); [cite: 21]

        if (!base64Data) return;

        // 2. إعداد بيانات الرفع
        const timestamp = Date.now(); [cite: 23]
        const fileName = `img_${timestamp}.png`; [cite: 23]
        const mimeType = 'image/png'; [cite: 23]

        // 3. رفع الصورة مباشرة إلى Vercel Blob
        const finalAttachmentUrl = await uploadToVercel(base64Data, fileName, mimeType); [cite: 24]

        // 4. عرض النتيجة في الشات
        console.log("تم الرفع بنجاح، الرابط:", finalAttachmentUrl); [cite: 25]
        setMessages(prev => [...prev, {
            id: Date.now(),
            text: `تم رفع صورة استشارية بنجاح: ${finalAttachmentUrl}`,
            sender: 'user',
            timestamp: new Date().toLocaleTimeString()
        }]); [cite: 25]
        ...
        <div style={styles.chatOverlay}>
          <div style={styles.chatBox}>
            <div style={styles.chatHeader}>
              <span><i className="fas fa-stethoscope"></i> استشاري الأطفال المعتمد</span>
            ...
            <div style={styles.chatInputArea}>
              ...
              <div style={styles.inputRow}>
                <input 
                  ...
                  placeholder="اسألي الاستشاري عن طفلكِ..."
                  style={styles.input}
                /> [cite: 43, 44]
                <button 
                  onClick={() => { 
