...
const PregnancyApp = () => {
  ...
  // ميزات إضافية: عداد الأسابيع والمنبه
  const [pregnancyWeek, setPregnancyWeek] = useState(12); // افتراضي الأسبوع 12
  const [reminders, setReminders] = useState(JSON.parse(localStorage.getItem('reminders')) || []);

  // المداخلات كاملة (10 حقول لكل قسم كما طلبتِ)
  const categories = [
  ...
  const runAiAnalysis = async (catIdx) => {
    ...
    try {
      const response = await HttpClient.post({
        ...
        headers: { 'Content-Type': 'application/json' },
        data: { 
          prompt: `أنتِ طبيبة نساء وتوليد خبيرة. حللي هذه البيانات لـ (${category.title}): ${dataString}. قدمي رداً موسعاً، دقيقاً طبياً، ومطمئناً بأسلوب "رقة".` 
        }
      });
  ...
  return (
    <div className="app-container" dir="rtl">
      ...
      <header className="header">
        <button className="ai-btn" onClick={() => setShowChat(true)}>طبيبة رقة <br/> AI</button>
      ...
      <div className="features-grid">
        ...
        <div className="feature-card" onClick={() => alert('سيتم تذكيرك بموعد الفحص القادم يوم الخميس')}>
                ...
                <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} style={{overflow:'hidden'}}>
                  <div className="inputs-container">
                    ...
                    <button className="analyze-full-btn" onClick={() => runAiAnalysis(index)}>
                      <Sparkles size={16} style={{display:'inline', marginLeft:'5px'}}/> {loading ? 'جاري التحليل...' : 'تحليل الطبيبة وحفظ'}
                    </button>
                  </div>
                </motion.div>
              )}
      ...
      <button style={{background:'#B91C1C', color:'white', border:'none', padding:'12px 40px', borderRadius:'30px', fontWeight:'bold', marginTop:'20px'}}>عودة</button>
          ...
          <motion.div className="chat-overlay" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}>
            <div className="chat-header">
              ...
              <div style={{textAlign:'center'}}>
                <div style={{fontWeight:'bold'}}>طبيبة رقة المتخصصة</div>
                <div style={{fontSize:'10px', opacity:0.8}}>تحليل بيانات الحمل والولادة</div>
              </div>
            ...
            <div className="chat-body">
              {loading && <div style={{textAlign:'center', padding:'20px', color:'#7C3AED', fontWeight:'bold'}}>انتظري قليلاً، الطبيبة تراجع ملفك...</div>}
              {chatHistory.map(msg => (
            ...
            <div style={{padding:'20px', borderTop:'1px solid #F1F5F9', display:'flex', gap:'10px'}}>
              <button className="ai-btn" style={{padding:'10px'}} onClick={()=>alert('فتح الكاميرا للأشعة...')}><Camera size={20}/></button>
              <button className="ai-btn" style={{padding:'10px'}} onClick={()=>alert('تسجيل صوتي للطبيبة...')}><Mic size={20}/></button>
              <div style={{flex:1, background:'#F8FAFC', borderRadius:'15px', display:'flex', alignItems:'center', px:15, fontSize:'11px', color:'#94A3B8', border:'1px solid #E2E8F0', padding:'0 15px'}}>اسألي الطبيبة عن أي عرض...</div>
