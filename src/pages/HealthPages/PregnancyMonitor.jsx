...
const PregnancyApp = () => {
  ...
  const [rawDeliveryISO, setRawDeliveryISO] = useState(''); // لتخزين الصيغة الخام للإرسال لـ نيون
  ...
  // دالة حساب تاريخ الولادة بنظام دقيق (قاعدة نيجل المعدلة للحمل المستمر)
  const calculatePregnancyStats = (month) => {
    ...
    // إضافة الأشهر المتبقية + 7 أيام (حسب النظام الشمسي المعتاد طبياً)
  ...
  // دالة الحفظ والمزامنة - تم ربطها بجدول "scheduled_for" في نيون
      ...
      if (savedToken) {
        await CapacitorHttp.post({
          ...
          data: {
            ...
            body: `تم حفظ تحليل ${categoryTitle} وجدولة موعد الولادة المتوقع.`,
  ...
  const runAiAnalysis = async (catIdx) => {
    ...
    if (!dataString) {
      alert("الرجاء إدخال البيانات للتحليل");
      return;
    }
  ...
  return (
    <div className="app-container" dir="rtl">
      <style>{`
        .app-container { background-color: #FDF2F8; min-height: 100vh; padding: 20px; font-family: 'Segoe UI', Tahoma, sans-serif; }
        .header { display: flex; justify-content: space-between; margin-bottom: 25px; align-items: center; }
        .ai-btn { background: #E9D5FF; color: #7C3AED; padding: 12px 20px; border-radius: 20px; border: none; cursor: pointer; font-weight: bold; display: flex; align-items: center; gap: 8px; }
        ...
        .baby-pointer-img {
          ...
          border: 4px solid white; background: #FDF2F8; top: -15px; left: calc(50% - 32.5px);
        }
        ...
        .category-item { background: white; border-radius: 25px; margin-bottom: 15px; border: 1px solid #FCE7F3; overflow: hidden; }
        .category-header { width: 100%; display: flex; justify-content: space-between; padding: 18px 20px; border: none; background: none; cursor: pointer; }
        .inputs-container { padding: 0 15px 15px; display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
        .custom-input { width: 100%; padding: 12px; border-radius: 12px; border: 1px solid #F1F5F9; background: #F8FAFC; }
      ...
      <div className="pregnancy-clock-section">
        <div className="clock-outer">
          ...
          >
            ...
            <div style={{width: 4, height: 100, background: '#DB2777', margin: '0 auto', marginTop: 30}}></div>
          </motion.div>
        ...
        {deliveryDate && (
          <div style={{marginTop: 20, color: '#7C3AED', fontWeight: 'bold', background: 'white', padding: '8px 20px', borderRadius: 20}}>
            موعد الولادة المتوقع (نيون): <span style={{color: '#DB2777'}}>{deliveryDate}</span>
          </div>
        )}
      </div>
      ...
      <div className="quick-chat-bar">
        ...
        <input className="quick-input" style={{flex:1, border:'none', outline:'none'}} placeholder="اسألي عن أي عرض..." value={promptInput} onChange={(e) => setPromptInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && sendCustomPrompt()} />
        ...
        {categories.map((cat, index) => (
          <div key={cat.id} className="category-item">
            <div className="category-header" onClick={() => setActiveTab(activeTab === index ? null : index)}>
              <ChevronRight style={{transform: activeTab === index ? 'rotate(90deg)' : 'none', color:'#7C3AED'}} />
              <div style={{display:'flex', alignItems:'center', gap:'12px'}}>
                ...
                <span style={{fontWeight:'bold'}}>{cat.title}</span>
                {cat.icon}
              </div>
            </div>
                ...
                <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }}>
                  <div className="inputs-container">
                    ...
                    <button className="analyze-full-btn" onClick={() => runAiAnalysis(index)}>
                      <Sparkles size={18}/> {loading ? 'جاري الحفظ في نيون...' : 'تحليل وحفظ التوقيت'}
                    </button>
                  </div>
                </motion.div>
              )}
        ...
        {showChat && (
          <motion.div className="chat-overlay" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}>
            <div className="chat-header">
              ...
              <div style={{fontWeight:'bold'}}>طبيب راقي</div>
            ...
            <div className="chat-body">
              {chatHistory.map(msg => (
                <div key={msg.id} className="msg-card">
                  <div style={{fontSize:'11px', color:'#7C3AED'}}>{msg.query}</div>
                  <p>{msg.reply}</p>
                </div>
              ))}
            </div>
