...
const MenstrualTracker = () => {
  ...
  const saveAndNotify = async (categoryTitle, currentAnalysis, customPrediction = null) => {
    const savedToken = localStorage.getItem('fcm_token');
    const activePrediction = customPrediction || prediction;
    
    let actualScheduledDate = new Date().toISOString(); 
    if (activePrediction && activePrediction.nextDate) {
        const dateParts = activePrediction.nextDate.split('/');
    ...
    const healthPayload = {
      ...
      note: `تحليل آلي لـ ${categoryTitle} | موعد القادمة: ${activePrediction?.nextDate || 'غير محدد'} | البداية: ${data['سجل التواريخ_تاريخ البدء']} | النهاية: ${data['سجل التواريخ_تاريخ الانتهاء']} | الخصوبة: ${activePrediction?.fertility || 'غير محدد'}`
    };
  ...
  const handleSaveAndAnalyze = async () => {
    ...
    try {
      ...
      // نمرر calc مباشرة لضمان استخدام التاريخ الجديد فوراً في الحفظ
