...
import { CapacitorHttp } from '@capacitor/core'; [cite: 1, 2]

const MenstrualTracker = () => {
  const HealthIcon = iconMap.health;

  // --- حالات البيانات --- [cite: 3]
  ...
  // --- جلب الإشعارات من Neon DB (CapacitorHttp.get) --- [cite: 7]
  ...
  // --- منطق المعالجة الرئيسي (حفظ في نيون + تحليل ذكاء اصطناعي عبر CapacitorHttp) --- [cite: 11]
  const handleProcess = async (userInput = null) => {
    setLoading(true);
    const summary = JSON.stringify(data); [cite: 12]
    
    try {
      // 1. مرحلة الحفظ في قاعدة بيانات نيون (Neon DB) 
      const saveOptions = {
        ...
        data: {
          ...
          value: summary,
          note: 'تم التحديث من واجهة المتابعة' [cite: 13]
        }
      };
      await CapacitorHttp.post(saveOptions); [cite: 14]

      // 2. مرحلة التحليل عبر API الذكاء الاصطناعي [cite: 14]
      ...
      const responseText = response.data.reply || response.data.message || "عذراً رقية، لم أتمكن من التحليل."; [cite: 19]
      ...
      // تحديث الإشعارات لرؤية النصيحة المحفوظة [cite: 23]
      await fetchNotifications(); [cite: 22]

    } catch (err) {
      console.error("فشل الاتصال الأصلي:", err);
      const errorMsg = { role: 'ai', content: "حدث خطأ في الشبكة، تأكدي من الاتصال بالإنترنت." }; [cite: 24]
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        {notifications.length > 0 && (
          <div style={{ background: '#FFF3E0', padding: '10px', borderRadius: '10px', marginBottom: '10px', fontSize: '12px', color: '#E65100' }}>
            🔔 نصيحة طبية: {notifications[0].body} [cite: 32]
          </div>
        )}
      ...
      {sections.map((sec) => (
        <div key={sec.id} style={styles.card}>
          <div onClick={() => setOpenAccordion(openAccordion === sec.id ? null : sec.id)} style={{ cursor: 'pointer', display: 'flex', justifyContent: 'space-between' }}>
            ...
            <span>{openAccordion === sec.id ? '▲' : '▼'}</span> [cite: 33, 34]
          </div>
      ...
      <button onClick={() => { setShowChat(true); handleProcess(); }} style={styles.btnPrimary}> [cite: 38]
        {loading ? "جاري الحفظ والتحليل..." : "حفظ وتحليل الحالة طبياً"} [cite: 39]
      </button>
      ...
      {showChat && (
        <div style={styles.chatOverlay}>
          <div style={{ padding: '20px', background: '#E91E63', color: '#fff', display: 'flex', justifyContent: 'space-between' }}>
            ...
            <button onClick={() => setChatHistory([])} style={{ background: 'none', border: 'none', color: '#fff' }}>مسح</button> [cite: 40]
          </div>
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px', background: '#FDF4F5' }}>
            {chatHistory.map((msg, i) => (
              <div key={i} style={{ 
                alignSelf: msg.role === 'user' ? 'flex-end' : 'flex-start',
                [cite_start]background: msg.role === 'user' ? '#E91E63' : '#fff', [cite: 41]
            ...
            {loading && <div style={{ textAlign: 'center', color: '#E91E63' }}>جاري التواصل مع الطبيبة...</div>}
          </div>
          <div style={styles.chatInputArea}> [cite: 43]
            <input 
              placeholder="اسألي طبيبة رقة عن دورتك وخصوبتك..." 
              style={{ flex: 1, border: 'none', padding: '12px', borderRadius: '20px' }}
              onKeyDown={(e) => { 
                if(e.key === 'Enter' && e.target.value.trim()) { 
                  handleProcess(e.target.value); [cite: 44]
